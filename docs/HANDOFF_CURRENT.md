...
`2f7626e` on `main`. `pytest`: 183 passed, 1 xfailed (DNS rebinding until #4 resolver policy).
...
- #7 DISCOVER/REFRESH, durable JSON scheduler, evidence-before-success, simulated 24-cycle soak, freshness windows, lease TTL
...
- #16 health probe + chaos restart + `ttc status` / `ttc properties` / `ttc doctor` / `ttc soak`
